...
public class StoredProcedureService {
  ...
  public VerkaufErgebnis verkaufErstellen(
      Long kundeId, Long filialeId, Long zahlungsartId, Long lagerId, String positionen) {
    log.info("SP_VERKAUF_ERSTELLEN kundeId={} filialeId={}", kundeId, filialeId);
    return jdbcTemplate.execute(
        (Connection con) -> {
          try (CallableStatement cs =
              con.prepareCall("{CALL SP_VERKAUF_ERSTELLEN(?,?,?,?,?,?,?)}")) {
    ...
    return jdbcTemplate.execute(
        (Connection con) -> {
          try (CallableStatement cs = con.prepareCall("{CALL SP_PRODUKT_SUCHEN(?,?,?,?,?,?)}")) {
            ...
            if (kategorieId != null) cs.setLong(2, kategorieId);
    ...
    return jdbcTemplate.execute(
        (Connection con) -> {
          try (CallableStatement cs =
            ...
            cs.execute();
            return new KundeErgebnis(cs.getLong(9), cs.getInt(10) == 1);
          }
        });
  }
